...
if [ ! -f /etc/systemd/system/com.redhat.Yggdrasil1.Worker1.echo.service ]; then
  ...
  yggctl generate worker-data   --name echo   --program /usr/libexec/yggdrasil/echo   --user root --output dbusfile_worker
  mv ~/go/bin/echo /usr/libexec/yggdrasil/echo
  cp dbusfile_echo/dbus-1/system.d/com.redhat.Yggdrasil1.Worker1.echo.conf /etc/dbus-1/system.d/
  cp dbusfile_echo/systemd/system/com.redhat.Yggdrasil1.Worker1.echo.service /etc/systemd/system/
  cp dbusfile_echo/dbus-1/system-services/com.redhat.Yggdrasil1.Worker1.echo.service /usr/share/dbus-1/system-services/
fi
