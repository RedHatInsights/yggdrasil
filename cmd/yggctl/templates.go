...
<busconfig>
    <policy group="{{ .User }}">
        <!-- Only {{ .User }} can own the Worker1.{{ .Name }} name. -->
        <allow own="com.redhat.Yggdrasil1.Worker1.{{ .Name }}" />

        <!-- Only {{ .User }} can send messages to the Worker1 interface. -->
        ...
        <!-- Only {{ .User }} can send messages to the Introspectable interface. -->
        <allow send_destination="com.redhat.Yggdrasil1.Worker1.{{ .Name }}"
            send_interface="org.freedesktop.DBus.Introspectable" />

        <!-- Only {{ .User }} can send messages to the Peer interface. -->
        <allow send_destination="com.redhat.Yggdrasil1.Worker1.{{ .Name }}"
