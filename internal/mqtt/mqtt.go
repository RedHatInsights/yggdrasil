...
func NewMQTTTransport(ClientID string, brokers []string, tlsConfig *tls.Config, controlHandler internal.CommandHandler, dataHandler internal.DataHandler) (*Transport, error) {
	t := Transport{
		ClientID: ClientID,
	}
	...
	mqttClientOpts.SetOnConnectHandler(func(client mqtt.Client) {
		...
		go internal.PublishConnectionStatus(&t, map[string]map[string]string{})
	})
...
func (t *Transport) handleDataMessage(msg mqtt.Message, handler internal.DataHandler) {
	log.Debugf("received a message %v on topic %v", msg.MessageID(), msg.Topic())
	handler(msg.Payload())
}

func (t *Transport) handleControlMessage(msg mqtt.Message, handler internal.CommandHandler) {
	log.Debugf("received a message %v on topic %v", msg.MessageID(), msg.Topic())
